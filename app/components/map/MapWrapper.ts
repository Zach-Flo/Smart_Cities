...
export default class MapWrapper {
  ...
  SetChoroplethView() {
    const choroplethView = document.getElementById(
      "choroplethView" + this.name
    );
  ...
  AddChoroplethLayer() {
    if (this.map == null || this.map.getLayer("ridesChoropleth")) {
      return;
    }
    this.map.addLayer({
      id: "ridesChoropleth",
      ...
      paint: {
        "fill-color": [
          ...
          ["get", "rides"],
          0,
          "#f7f7f7", // Color for lowest frequency
          2,
          "#ffffb2", // Color for low frequency
          5,
          "#fed976", // Color for moderate frequency
          10,
          "#feb24c", // Color for higher frequency
          15,
          "#fd8d3c", // Color for high frequency
          20,
          "#fc4e2a", // Color for very high frequency
          25,
          "#e31a1c", // Color for extremely high frequency
          30,
          "#bd0026", // Color for maximum frequency
        ],
  ...
  AddRegionsLayer() {
    if (this.map == null || this.map.getLayer("ridesFreq1")) {
      return;
    }
    this.map.addLayer({
      id: "ridesFreq1",
  ...
  AddBordersLayer() {
    if (this.map == null || this.map.getLayer("ridesBorders")) {
      return;
    }
    this.map.addLayer({
  ...
  SwitchToExtrusion() {
    if (this.map == null || this.map.getLayer("rides3D")) {
      return;
    }
    this.map.flyTo({
    ...
    this.map.addLayer({
      ...
      paint: {
        ...
        "fill-extrusion-height": [
          ...
          ["get", "rides"],
          0,
          0, // Height for lowest frequency
          80,
          5000, // Maximum height for maximum frequency (adjust as needed)
        ],
